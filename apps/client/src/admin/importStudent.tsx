...
export default function ImportStudent() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile.size > MAX_SIZE_BYTES) {
      setErrors([{ message: `Fichier trop volumineux. Taille max: ${Math.round(MAX_SIZE_BYTES / (1024 * 1024))}MB` }]);
      setFile(null);
      return;
    }

    if (selectedFile.type && !ALLOWED_MIME_TYPES.includes(selectedFile.type)) {
      setErrors([{ message: `Type MIME non autorisé (${selectedFile.type}). Autorisés: ${ALLOWED_MIME_TYPES.join(', ')}` }]);
      setFile(null);
      return;
    }
        ...
        <div className="alert alert-error mb-6">
          ...
          <div>
            ...
            <ul className="list-disc list-inside">
              <li>Échec de l’import : certaines données existent déjà.</li>
            </ul>
            {!skipExisting && (
              <div className="mt-2 text-sm">
                <strong>Astuce:</strong> Cochez l'option "Ignorer les enregistrements existants" pour Ignorer automatiquement les doublons.
              </div>
            )}
          </div>
        </div>
