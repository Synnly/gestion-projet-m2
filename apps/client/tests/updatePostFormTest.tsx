...
describe('UpdatePost (form + loader)', () => {
    ...
    it("redirige vers le dashboard si le post n'existe pas (loader)", async () => {
        ...
        await expect(updatePostLoader({ params: { postId: 'missing' } } as any)).rejects.toHaveProperty('status', 302);
    ...
    it("redirige vers le dashboard si le post n'appartient pas à la compagnie (403)", async () => {
        ...
        await expect(updatePostLoader({ params: { postId: 'other' } } as any)).rejects.toHaveProperty('status', 302);
        expect(fetchMock).toHaveBeenCalled();
    });
});
