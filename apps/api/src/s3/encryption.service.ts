...
export class EncryptionService {
    ...
    constructor(private readonly configService: ConfigService) {
        ...
        if (!masterKeyBase64) {
            ...
            try {
                ...
                throw new Error(
                    `Invalid STORAGE_MASTER_KEY format: ${error.message}. Expected base64-encoded 256-bit key`,
                );
            }
        }
    }
     ...
     */
    private decryptDataKey(encryptedKey: Buffer, iv: Buffer, authTag: Buffer): Buffer {
    ...
    encrypt(data: Buffer): {
        ...
        const { encryptedKey: dekEncrypted, iv: dekIv, authTag: dekAuthTag } = this.encryptDataKey(dataKey);
    ...
    decrypt(
        ...
        try {
            try {
                ...
                const decipher = crypto.createDecipheriv(this.CONSTANTS.algorithm, dataKey, dataIv);
