...
describe('FileValidationPipe', () => {
    ...
    describe('transform', () => {
        ...
        it('should reject filename ending with dot (line 42 coverage)', async () => {
            const dto = {
                originalFilename: 'file.',
                fileType: 'logo',
            };

            // 'file.' split by '.' gives ['file', ''] and pop() gives '' which is falsy
            await expect(pipe.transform(dto)).rejects.toThrow(BadRequestException);
...
describe('FileValidator', () => {
    describe('sanitizeSvg', () => {
        ...
        it('should throw error if SVG is invalid after sanitization', () => {
            ...
            expect(() => FileValidator.sanitizeSvg(invalidSvg)).toThrow('Invalid SVG content after sanitization');
        });
        ...
        it('should throw error if SVG contains forbidden elements that patterns miss', () => {
            ...
            expect(() => FileValidator.sanitizeSvg(svgWithForbidden)).toThrow('SVG contains forbidden elements');
        });
    ...
    describe('validateMimeType', () => {
        ...
        it('should reject invalid MIME type for logo', () => {
            ...
            expect(() => FileValidator.validateMimeType(invalidMimeType, 'logo')).toThrow('Invalid MIME type for logo');
        });

        it('should reject invalid MIME type for CV', () => {
            const invalidMimeType = 'image/png';

            expect(() => FileValidator.validateMimeType(invalidMimeType, 'cv')).toThrow('Invalid MIME type for cv');
        });
    });
});
