...
describe('UserService', () => {
    ...
    describe('ban', () => {
        it('should ban a user, delete refresh tokens and send an email', async () => {
            ...
            expect(mailerService.sendAccountBanEmail).toHaveBeenCalledWith(mockUser.email, 'Comportement inapproprié');
        });
        ...
        it('should throw NotFoundException if user is not found or already banned', async () => {
            ...
            await expect(service.ban('user123', 'reason')).rejects.toThrow(NotFoundException);
